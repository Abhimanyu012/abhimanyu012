...
- Tournament management with automated draw generation and live payment integration.
- Decoupled backend (Express/Drizzle ORM) and frontend (React/TypeScript) deployments on Vercel.
- Secure Razorpay payment flow with webhook verification.
...
- Study-abroad guidance with AI-powered university recommendations.
- Dual AI model system (Google GenAI + Groq fallback) for reliable responses.
- Relational PostgreSQL schema optimized with foreign keys and indexes.
- JWT auth, bcryptjs hashing, and secure middleware for protected routes.
...
- Marketplace for listing services, placing orders, and reviews.
- Index-optimized queries for Users, Gigs, Orders, and Reviews.
- RBAC for strict access control in the API.
